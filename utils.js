...
(async () => {
    ...
    for (let j = 0; j < numberOfPairs; j++) {
        console.log(`Working on ${j + 1} pair`);
        let axes, declarers, contracts, tricks;
        try {
            const data = await getPairData(j + 1);
            axes = data.axes;
            declarers = data.declarers;
        ...
        for (let i = 0; i < axes.length; i++) {
            if (contracts[i]) {
                ...
                let dealData;
                try {
                    dealData = await getDealData(i + 1);
                } catch (err) {
                    console.log(err.message);
                    i--;
                    continue;
        ...
        wStream.write(`${j + 1};${attack};${attackCounter};${attack / attackCounter};${defence};${defenceCounter};${defence / defenceCounter};${attackLevelSum / attackCounter};${defenceLevelSum / defenceCounter};${contractsMade / attackCounter};${contractsDefeated / defenceCounter}\n`);
